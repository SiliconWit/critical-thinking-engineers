...
```text
  Logical Fallacies: Categories
  ──────────────────────────────────────────────────────────
  ATTACK THE PERSON      DISTORT THE ARGUMENT   FALSE REASONING
  ─────────────────────  ─────────────────────  ──────────────
  Ad Hominem             Straw Man              False Dilemma
  (dismiss by attacking) (misrepresent)         (only two options)

  Genetic Fallacy        Red Herring            Slippery Slope
  (judge by origin)      (change subject)       (cascade disaster)

  Appeal to Authority    Circular Reasoning     Hasty Generalization
  (trust the person)     (beg the question)     (one case = rule)
```
...
```mermaid
flowchart TD
    A[Evaluate the argument] --> B{Attacking a person\nnot the idea?}
    B -- Yes --> C[Ad Hominem / Genetic Fallacy]
    B -- No --> D{Position accurately\nrepresented?}
    D -- No --> E[Straw Man]
    D -- Yes --> F{Only two\noptions presented?}
    F -- Yes --> G[False Dichotomy]
    F -- No --> H{Data cherry-picked?}
    H -- Yes --> I[Texas Sharpshooter]
    H -- No --> J{Good solution rejected\nbecause not perfect?}
    J -- Yes --> K[Nirvana Fallacy]
    J -- No --> L[Likely sound argument]
    style L fill:#f0fdf4,stroke:#84cc16
```
